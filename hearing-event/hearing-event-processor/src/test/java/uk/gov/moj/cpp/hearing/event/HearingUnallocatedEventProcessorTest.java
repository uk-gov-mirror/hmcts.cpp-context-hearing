...
public class HearingUnallocatedEventProcessorTest {
    ...
    @Test
    public void shouldHandleHearingUnallocatedPublicEvent() {
        final String hearingId = randomUUID().toString();
        final String offenceId = randomUUID().toString();
        final JsonObject hearingUnallocated = createObjectBuilder()
                .add("hearingId", hearingId)
                .add("offenceIds", createArrayBuilder()
                        .add(offenceId.toString())
                        .build())
                .build();
        ...
        assertThat(commandEvent.metadata().name(), is("hearing.command.unallocate-hearing"));
    ...
    @Test
    public void shouldHandleHearingUnallocatedPrivateEvent() {
        ...
        this.hearingUnallocatedEventProcessor.handleHearingUnallocatedPrivateEvent(envelopeFrom(metadataWithDefaults().build(), createObjectBuilder()
        ...
        verify(this.sender, times(3)).send(this.publicEventArgumentCaptor.capture());

        final Envelope<JsonObject> caseCommand = this.publicEventArgumentCaptor.getAllValues().get(0);
        assertThat(caseCommand.metadata().name(), is("hearing.command.remove-hearing-for-prosecution-cases"));
        ...
        assertThat(defendantCommand.metadata().name(), is("hearing.command.remove-hearing-for-defendants"));
        ...
        assertThat(offenceCommand.metadata().name(), is("hearing.command.remove-hearing-for-offences"));
    ...
    @Test
    public void shouldHandleHearingUnallocatedPrivateEventWhenProsecutionCaseIdAndDefendantIdsAreEmpty() {
        final UUID hearingId = randomUUID();
        final UUID offenceId1 = randomUUID();
        ...
        final Envelope<JsonObject> offenceCommand = this.publicEventArgumentCaptor.getValue();
        assertThat(offenceCommand.metadata().name(), is("hearing.command.remove-hearing-for-offences"));
        final List<JsonString> actualOffenceIds = offenceCommand.payload().getJsonArray("offenceIds").getValuesAs(JsonString.class);
        assertThat(actualOffenceIds.size(), is(1));
        assertThat(actualOffenceIds.get(0).getString(), is(offenceId1.toString()));
    }
